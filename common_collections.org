...
fn main() {
    let query1 = "Add Sally to Engineering";
    let query2 = "Add Amir to Sales";

    // make a parser that gets the name, dept from sentence
    // get dept
    let get_dept = parse_up_to("to ");
    // get name
    let up_to_ws = parse_up_to(" ");

    println!(
        "{:?}, {:?}",
        get_dept(query1).unwrap(),
        up_to_ws(query1).unwrap()
    );
    // add to hashmap as key value pair
}
#+END_SRC
